...
class DDPG:
    def __init__(self, n_states, n_hiddens, n_actions, action_bound,
        ...
        self.actor.load_state_dict(torch.load('TargetActor4ini.pth'))
        # Critic Network: Target
        self.critic = QValueNet(n_states, n_hiddens, n_actions).to(device)
        self.critic.load_state_dict(torch.load('TargetCritic4ini.pth'))
    ...
    def take_action(self, state):
        ...
        actions = self.actor(state).detach().numpy()#.detach().cpu().numpy()  # Assuming the output is on GPU
        #actions += self.sigma * np.random.randn(self.n_actions)  # Adding noise element-wise
        #actions = self.actor(state).item()
        return actions
