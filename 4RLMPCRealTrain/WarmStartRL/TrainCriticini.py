...
for i in range(4000):  # Iteration for n episode
    ...
    # For each episode, Do RLMPC (RL with initialized MPC actor network)
...
torch.save(agent.actor.state_dict(), 'TrainActoriniReal1.pth')  # Saves only the model parameters
torch.save(agent.target_actor.state_dict(), 'TargetActoriniReal1.pth') 
torch.save(agent.critic.state_dict(), 'TrainCriticiniReal1.pth') 
torch.save(agent.target_critic.state_dict(), 'TargetCriticiniReal1.pth') 
